...
"This file is an unedited version of one of the R programs used in the dissonance study. 
 ...
 "
  mat1<-function(n1,n2,method){
   ...
   v01<-jarque.bera.test(sample1)$statistic
   v02<-jarque.bera.test(sample1)$p.value
   ...
   v1<-jarque.bera.test(sample2)$statistic
   v2<-jarque.bera.test(sample2)$p.value
  ...
  write.csv(ma_df, file = "d:\\sample own size itereate JB.csv", row.names = FALSE)
